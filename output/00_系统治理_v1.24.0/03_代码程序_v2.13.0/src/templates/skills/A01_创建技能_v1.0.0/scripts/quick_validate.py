...
def get_yaml_scalar_value(text: str, key: str) -> str | None:
...
def validate_skill(skill_root: Path) -> tuple[list[str], list[str]]:
    ...
    if not SKILL_NAME_PATTERN.match(folder_name):
        errors.append("Skill 文件夹名必须符合 {域代码}{编号}_{技能名}_v{MAJOR}.{MINOR}.{PATCH}")
    ...
    if not name:
        ...
        errors.append(f"frontmatter name 必须与文件夹名一致：{folder_name}")
    elif not SKILL_NAME_PATTERN.match(name):
        errors.append("frontmatter name 必须带域编号和三段式版本号")
    elif not re.search(r"[\u4e00-\u9fff]", name):
        errors.append("frontmatter name 应包含中文")
    if not description:
        errors.append("frontmatter 缺少 description")
    elif len(description) > 100:
        errors.append(f"frontmatter description 超过 100 字：{len(description)}")
    ...
    if not agents_yaml.exists():
        ...
        display_name = get_yaml_scalar_value(yaml_text, "display_name")
        short_description = get_yaml_scalar_value(yaml_text, "short_description")
        if not display_name:
            errors.append("agents/openai.yaml 缺少 display_name")
        elif display_name != folder_name:
            errors.append(f"agents/openai.yaml display_name 必须与文件夹名一致：{folder_name}")
        if not short_description:
            errors.append("agents/openai.yaml 缺少 short_description")
