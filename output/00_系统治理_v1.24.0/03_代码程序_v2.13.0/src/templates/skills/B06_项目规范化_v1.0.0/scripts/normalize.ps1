    ...
    if (Test-Path -LiteralPath $outputRoot) {
        ...
        foreach ($project in $coreProjects) {
            $codeDirs = @(Get-ChildItem -LiteralPath $project.FullName -Directory -ErrorAction SilentlyContinue |
                Where-Object { $_.Name -match '^03_代码程序_v\d+\.\d+\.\d+$' })
...
function Get-SkillNamingContractProblems {
    ...
    $skillNamePattern = '^[A-Z]\d{2}_[^\\/:*?"<>|]+_v\d+\.\d+\.\d+$'
    ...
    foreach ($dir in $existingSkills) {
        ...
        if (-not (Test-Path -LiteralPath $openAiYaml)) {
            ...
            } elseif ($displayName -ne $folderName) {
...
function Ensure-VersionedContentFileName($fileName) {
    ...
    if ($stem -match '_v\d+\.\d+\.\d+$') {
        return "$stem$ext"
    }
    return "${stem}_v1.0.0$ext"
}
...
if (Test-Path $agents) {
    $requiredSkills = if ($workspaceSpec) { @($workspaceSpec.overwriteLayer.skills) } else { @("A01_创建技能_v1.0.0","B04_子项目管理_v1.0.0","A02_安装技能_v1.0.0","B01_框架体检_v1.0.0","B02_版本控制备份_v1.0.0","B03_记忆管理_v1.0.0","B06_项目规范化_v1.0.0","B05_课题研究_v1.1.0") }
    ...
    if (-not (Test-Path $skillsDir)) {
        ...
        foreach ($item in $skillNamingProblems.BadDisplayName) { Write-Host "  [FAIL] OpenAI display_name must equal folder name: $item"; $issues++ }
        ...
        if ($isStructured) {
            ...
            foreach ($spec in $projectSubFolderSpecs) {
                ...
                if ($spec.Prefix -ne "03") {
                    ...
                    $unversioned = @(Get-SubFolderContentFiles $primary.FullName |
                        Where-Object { $_.Name -notmatch '_v\d+\.\d+\.\d+\.[^.]+$' })
                    if ($unversioned.Count -gt 0) {
                        Write-Host "  [WARN] Versioned content files required in $($d.Name)/$($primary.Name): $($unversioned.Count)"
                        $warnings++
                    }
                }
            }
