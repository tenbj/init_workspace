...
| `AGENTS.md`、`CLAUDE.md` 根入口文件 | CONFIG | `-Mode CONFIG` | **保持不变**，分别备份到 `.history/AGENTS/`、`.history/CLAUDE/` |
| `.agents/skills/` 下的 Skill 文件夹 | FOLDER | `-Mode FOLDER` | **保持不变** |
| `.system/` 下的子文件夹 | FOLDER | `-Mode FOLDER` | **保持不变** |
...
```powershell
powershell -ExecutionPolicy Bypass -File ".agents\skills\B02_版本控制备份_v1.0.0\scripts\backup.ps1" `
...
```powershell
powershell -ExecutionPolicy Bypass -File ".agents\skills\B02_版本控制备份_v1.0.0\scripts\backup.ps1" `
    -TargetPath "output\05_工作区初始化工具_v1.2.0" `
    -ChangeType MINOR
```
...
#### ⚙️ CONFIG 模式 — 配置文件与根入口文件

原文件**保持不变**，仅在 `.history/` 复制一份带时间戳的备份。`.agents/rules/` 按镜像路径进入 `.history/.agents/rules/`；`AGENTS.md` 与 `CLAUDE.md` 分别进入 `.history/AGENTS/` 与 `.history/CLAUDE/`。
...
**FOLDER 模式输出示例：**
```
[OK] Skill folder snapshot created: .history\.agents\skills\B02_版本控制备份_v1.0.0_202605031718
==========================================
  Folder Snapshot Complete
==========================================
  Source   : .agents\skills\B02_版本控制备份_v1.0.0
  Snapshot : .history\.agents\skills\B02_版本控制备份_v1.0.0_202605031718
==========================================
```
...
└── .history\                                 ← 备份目录（镜像结构）
    ...
    │   └── skills\
    │       └── B02_版本控制备份_v1.0.0_202605031718\               ← FOLDER 快照（时间戳）
    └── .memory\
        ├── 知识提炼\
