...
用法：
    python generate_dwd_sql.py <ods_table_name> --output-dir <输出目录>

示例：
    python generate_dwd_sql.py cbebg.ods_realtime_db_t_lx_newad_hasadgroups --output-dir output/01_DWD开发产物_v1.0.0/03_代码程序_v1.0.0

输出：
    <output-dir>/<dwd_table_name>/<dwd_table_name>.sql
    <output-dir>/<dwd_table_name>/<dwd_table_name>.md

注意：--output-dir 为必填参数，不指定时脚本报错退出。
"""
...
# 脚本位于 .agents/skills/C05_1对1DWD单表SQL生成_v1.0.0/scripts/，向上 4 级到项目根目录
PROJECT_ROOT = SCRIPT_DIR.parents[3]
# OUTPUT_DIR 不再提供默认值，必须通过 --output-dir 参数指定
# 避免绕过子项目管理 Skill 在 output/ 下创建不合规文件夹

# ODS SQL 源码候选目录，按顺序取第一个存在的
_DATA_ASSETS_CANDIDATES = [
    PROJECT_ROOT / "权威资料" / "data-assets",
    PROJECT_ROOT / "data-assets",
    PROJECT_ROOT / "input" / "data-assets",
]
DATA_ASSETS_DIR = next((p for p in _DATA_ASSETS_CANDIDATES if p.exists()), _DATA_ASSETS_CANDIDATES[0])
...
COMPACT_ENUM_SUFFIXES = tuple(sfx.lstrip("_") for sfx in ENUM_SUFFIXES)
...
DIM_SHOP_REPORT_FIELDS = (
    "sid", "lx_shop_id", "shopid", "saas_shop_id",
    "seller_id", "sellerid",
)
DIM_SHOP_NAME_FIELDS = ("shopname", "acc_name", "shop", "account")
DIM_SHOP_SITE_FIELDS = ("sitename", "country", "site", "country_code")
DIM_EVIDENCE_FIELDS = set(
    DIM_SHOP_REPORT_FIELDS
    + DIM_SHOP_NAME_FIELDS
    + DIM_SHOP_SITE_FIELDS
    + ("platform", "asin", "sku", "msku")
)

# ── cbebg 下的域目录名（仅用于路径提取失败时的 fallback 提示，不参与匹配逻辑）
CBEBG_SCHEMA = "cbebg"
...
def is_enum_candidate(name: str) -> bool:
    """
    Detect enum-like fields before full LLM word-splitting.

    The script only performs camelCase/PascalCase to snake_case. Compact ODS
    names such as settlementstatus or accounttype are split later by the LLM,
    so enum detection must also match compact suffixes.
    """
    n = name.lower()
    if any(n.endswith(sfx) for sfx in ENUM_SUFFIXES):
        return True
    return any(n.endswith(sfx) for sfx in COMPACT_ENUM_SUFFIXES)
...
def format_list(items: list[str] | tuple[str, ...]) -> str:
    return "、".join(items) if items else "无"


def find_ods_sql(table_name: str) -> Path | None:
    bare = strip_schema(table_name).lower()
    for pattern in [f"{bare}.sql", f"*{bare}*.sql", bare, f"*{bare}*"]:
        hits = [p for p in DATA_ASSETS_DIR.rglob(pattern) if p.is_file()]
        if hits:
            return hits[0]
    return None
...
def derive_domain_from_path(sql_path: Path) -> str | None:
    """
    从文件路径中提取域名。
    路径结构：.../data-assets/cbebg/{域}/...
    找到 cbebg 目录后取下一级目录名即为域。
    """
    parts = sql_path.parts
    for i, part in enumerate(parts):
        if part == CBEBG_SCHEMA and i + 1 < len(parts):
            return parts[i + 1]
    return None


def derive_dwd_name(ods_table: str, strategy: str, sql_path: Path) -> str:
    ...
    domain = derive_domain_from_path(sql_path)
    if domain:
        print(f"     → 域来自路径：{domain}")
    else:
        domain = "unknown"
        print(f"    ⚠️  路径中未找到 cbebg/{{域}} 结构，域设为 '{domain}'，请人工确认")

    suffix = "ilu" if strategy == "ilu" else "flu"
    return f"{TARGET_SCHEMA}.dwd_{domain}_{biz}_{suffix}_dd"
...
def build_dim_detail_lines(
    evidence_fields: list[str],
    source_norms: list[str],
    dim_archive: str | None,
    dim_src_field: str | None,
    dim_dim_field: str | None,
    has_asin: bool,
) -> list[str]:
    norm_set = set(source_norms)
    lines: list[str] = []

    evidence = unique_keep_order(evidence_fields)
    if evidence:
        lines.append(f"已扫描到疑似业务关联字段：{format_list(evidence)}")
    else:
        lines.append("未扫描到明显维表候选字段")

    if dim_archive and dim_src_field and dim_dim_field:
        lines.append(
            f"已命中标准店铺维表信号：{dim_src_field} -> {dim_archive}，"
            f"自动关联 {DIM_SHOP_TABLE} 并补充 shop_id"
        )
    else:
        shop_signal_hits = [f for f in DIM_SHOP_REPORT_FIELDS if f in norm_set]
        if shop_signal_hits:
            lines.append(
                f"检测到店铺维表候选字段：{format_list(shop_signal_hits)}，"
                "但未形成脚本可自动 JOIN 的完整条件，需人工确认"
            )
        else:
            lines.append(
                "未扫描到标准店铺维表强命中字段："
                f"{format_list(DIM_SHOP_REPORT_FIELDS)}"
            )

    name_hits = [f for f in DIM_SHOP_NAME_FIELDS if f in norm_set]
    site_hits = [f for f in DIM_SHOP_SITE_FIELDS if f in norm_set]
    has_platform = "platform" in norm_set
    if not dim_archive and name_hits and site_hits and has_platform:
        lines.append(
            f"{format_list(name_hits)} + platform + {format_list(site_hits)} "
            "理论上接近 DIM-SHOP-003 的三字段联合关联模式，"
            "但字段语义可能存在歧义，因此未自动补充 JOIN"
        )
        lines.append(
            f"TODO_CONFIRM: 如确认本表可按账号/店铺 + 平台 + 站点/国家关联 {DIM_SHOP_TABLE}，"
            "请补充 dim_shop.shop_id，并确认字段对应关系"
        )
    elif not dim_archive and (name_hits or site_hits or has_platform):
        lines.append("检测到部分店铺维表候选字段，但不足以组成标准 DIM-SHOP-003 组合，未自动补充 JOIN")

    if has_asin:
        if dim_archive:
            lines.append("检测到 asin 且已获得 shop_id，疑似可关联 DIM-MSKU-001，需人工确认是否补充 msku_id、msku")
        else:
            lines.append("检测到 asin，但未先获得 shop_id；如需关联 MSKU 维表，请确认账号/站点组合或店铺维表关联")
    else:
        lines.append("未扫描到标准 MSKU 维表强命中字段：asin")
...
def build_sql(
    ...
    if timestamp_fields:
        header_lines.append(
            f"-- ⚠️ TIMESTAMP_FIELDS: {', '.join(timestamp_fields)}\n"
            f"-- LLM请核查以上字段的 ODS 类型后决定是否需要 from_unixtime 转换"
        )
    for w in warnings:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        required=True,
        help="根输出目录（必填）。调用前先通过子项目管理定位 DWD 开发产物子项目，"
             "传入其 03_代码程序_v* 路径。",
    )
    ...
    # Step 3: DWD 表名
    print("[3/6] 推导 DWD 表名")
    dwd_table = derive_dwd_name(ods_table, strategy, sql_path)
    print(f"     → {dwd_table}")
    ...
    for raw in source_exprs:
        ...
        if norm in DIM_EVIDENCE_FIELDS:
            dim_evidence_fields.append(alias)

        # 时间戳字段检测（FIELD-006）：不在脚本中转换，仅标记供 LLM 阶段处理
        ...
        if (
            ...
            timestamp_fields.append(norm)
            print(f"     [FIELD-006] 疑似时间戳字段（待 LLM 核查 ODS 类型后处理）：{norm}")
        ...
        # 枚举字段检测：同时支持已拆分和全小写紧凑字段
        if is_enum_candidate(norm):
    ...
    dim_detail_lines = build_dim_detail_lines(
        evidence_fields = dim_evidence_fields,
        source_norms    = source_norms,
        dim_archive     = dim_archive,
        dim_src_field   = dim_src_field,
        dim_dim_field   = dim_dim_field,
        has_asin        = has_asin,
    )
...
# ── 版本更新记录 ───────────────────────────────────────────────────────────────
# v1.0  初始版本
# v1.1  DATA_ASSETS_DIR 改为多路径候选（权威资料/data-assets → data-assets → input/data-assets）
# v1.2  域识别改为从 SQL 文件路径提取（data-assets/cbebg/{域}/...），废弃硬编码前缀匹配
# v1.3  枚举候选检测支持全小写紧凑字段（如 settlementstatus/accounttype）
# v1.4  .md 固定输出维表判断明细，记录候选字段、命中结果、未 JOIN 原因和人工确认点
