...
$requiredSkills = if ($spec) {
    ...
    @("A01_创建技能_v1.0.0","B04_子项目管理_v1.0.0","A02_安装技能_v1.0.0","B01_框架体检_v1.0.0","B02_版本控制备份_v1.0.0","B03_记忆管理_v1.0.0","B06_项目规范化_v1.0.0","B05_课题研究_v1.1.0")
}
...
if ($requiredDirs.Count -gt 0) {
    ...
    foreach ($item in $skillNamingProblems.BadDisplayName) {
        Write-Host "  [FAIL] OpenAI display_name must equal folder name: $item"
        $issues++
    }
    ...
    if ($skillNamingProblems.BadFolderNames.Count -eq 0 -and
        ...
        Write-Host "  [PASS] Skill folders, YAML names, OpenAI display names, descriptions, and registry align ($($registeredSkills.Count))"
        $passes++
    }
    Write-Host ""
}
