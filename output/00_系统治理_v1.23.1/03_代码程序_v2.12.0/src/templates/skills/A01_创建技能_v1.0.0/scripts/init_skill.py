...
def skill_title_from_name(skill_name: str) -> str:
    match = re.match(r"^[A-Z]\d{2}_(.+)_v\d+\.\d+\.\d+$", skill_name)
...
def validate_skill_name(skill_name: str) -> None:
    ...
    if not SKILL_NAME_PATTERN.match(skill_name):
        raise ValueError("skill 名必须符合 {域代码}{编号}_{技能名}_v{MAJOR}.{MINOR}.{PATCH}，例如 F01_原子拆解技能_v1.0.0。")
    if not INITIAL_VERSION_PATTERN.search(skill_name):
        raise ValueError("新建 skill 默认版本必须为 v1.0.0。")
...
def main() -> int:
    ...
    parser.add_argument("skill_name", help="标准 Skill 目录名，同时也是 frontmatter name，例如 F01_原子拆解技能_v1.0.0")
    parser.add_argument(
        "--path",
