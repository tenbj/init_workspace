...
3. 用 `scripts/init_skill.py` 按 `{域代码}{编号}_{技能名}_v1.0.0` 生成或补齐标准骨架
4. 把 `SKILL.md` 收口成薄入口，只保留触发、动作骨架、references 路由和边界
5. 用 `scripts/generate_openai_yaml.py` 生成或更新 `agents/openai.yaml`，其中 `display_name` 必须等于完整标准 Skill 名
