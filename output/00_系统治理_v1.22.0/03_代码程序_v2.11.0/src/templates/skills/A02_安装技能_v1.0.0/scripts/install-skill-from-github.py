...
SKILL_NAME_PATTERN = re.compile(r"^[A-Z]\d{2}_[^\\/:*?\"<>|]+_v\d+\.\d+\.\d+$")
INITIAL_VERSION_PATTERN = re.compile(r"_v1\.0\.0$")
...
def validate_standard_skill_name(name: str) -> str:
    candidate = name.strip()
    if any(sep in candidate for sep in ("/", "\\")):
        raise InstallError("本地 skill 标准名不能包含路径分隔符。")
    if not SKILL_NAME_PATTERN.match(candidate):
        raise InstallError("本地 skill 名必须符合 {域代码}{编号}_{技能名}_v{MAJOR}.{MINOR}.{PATCH}，例如 F01_原子拆解技能_v1.0.0。")
    if not INITIAL_VERSION_PATTERN.search(candidate):
        raise InstallError("安装为新本地 skill 时，默认版本必须为 v1.0.0。")
    if not contains_chinese(candidate):
        raise InstallError("本地 skill 名中的技能名应以中文为主，可包含必要英文术语。")
    return candidate


def display_name_from_standard_name(name: str) -> str:
    return validate_standard_skill_name(name)
...
def build_thin_description(source_name: str) -> str:
    base = f"当用户需要复用“{source_name}”相关能力时使用；提供本地中文薄入口、原始说明和迁移记录。"
    if len(base) <= 100:
        return base
    return "当用户需要复用外部 Skill 能力时使用；提供本地中文薄入口、原始说明和迁移记录。"


def thin_skill_markdown(standard_name: str, display_name: str, source_name: str, source_description: str) -> str:
    description = build_thin_description(source_name)
    return f"""---
name: {standard_name}
...
def localize_installed_skill(source: Source, source_path: str, source_dir: Path, dest_root: Path, requested_name: str | None) -> Path:
    ...
    if not requested_name:
        raise InstallError("安装到当前项目时必须通过 --name 指定标准 Skill 名，例如 F01_原子拆解技能_v1.0.0。")
    standard_name = validate_standard_skill_name(requested_name)
    display_name = display_name_from_standard_name(standard_name)
    dest_dir = dest_root / standard_name
    ...
    (dest_dir / "SKILL.md").write_text(
        thin_skill_markdown(standard_name, display_name, source_name, source_description),
        encoding="utf-8",
    )
    ...
    write_openai_yaml(dest_dir, display_name)
...
def parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument("--name", help="本地标准 Skill 名，必须形如 F01_原子拆解技能_v1.0.0")
