...
def latest_model_project() -> Path | None:
    ...
    candidates = [path for path in root.glob(f"*_{MODEL_PROJECT_TOPIC}_v*") if path.is_dir()]
    return max(candidates, key=subproject_version_key) if candidates else None
...
def ensure_default_code_root() -> Path:
    ...
    if not project:
        raise FileNotFoundError(
            "Cannot find output/*_模型设计标准化_v*. Run B04 to create or locate the 模型设计标准化 subproject first."
        )

    code_dirs = sorted(project.glob("03_代码程序_v*"))
    if not code_dirs:
        code_dir = project / "03_代码程序_v1.0.0"
