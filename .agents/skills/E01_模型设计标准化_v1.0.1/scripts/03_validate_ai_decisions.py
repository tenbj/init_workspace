...
def validate_field_decisions(run_dir: Path, allow_missing: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    expected = {(f["sheet_name"], int(f["row_number"])) for f in fields}
    ...
    for idx, row in enumerate(decisions, 1):
        ...
        if writeback_allowed and (decision != "fill_safe" or confidence != "high" or not recommended):
            issues.append(issue("error", "field_decision", "unsafe_writeback", "writeback_allowed=true 仅允许 high 置信度 fill_safe 且必须有推荐字段名", sheet_name=sheet, row_number=row_number))
...
def main() -> None:
    ...
    decisions, field_issues, review_items = validate_field_decisions(run_dir, args.allow_missing)
