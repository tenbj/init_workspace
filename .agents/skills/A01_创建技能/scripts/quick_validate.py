...
def validate_skill(skill_root: Path) -> tuple[list[str], list[str]]:
    ...
    if not SKILL_NAME_PATTERN.match(folder_name):
        errors.append("Skill 文件夹名必须符合 {域代码}{编号}_{技能名}，不得带版本号")
    ...
    if not name:
        ...
        errors.append("frontmatter name 必须是稳定 Skill 名，不带版本号")
    ...
    if not agents_yaml.exists():
        ...
        elif not re.match(rf"^{re.escape(folder_name)}_v\d+\.\d+\.\d+$", display_name):
            errors.append(f"agents/openai.yaml display_name 必须为 {folder_name}_vMAJOR.MINOR.PATCH")
