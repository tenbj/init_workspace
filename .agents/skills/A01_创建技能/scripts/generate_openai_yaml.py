...
def parse_interface_overrides(raw_overrides: list[str]) -> tuple[dict[str, str], list[str]]:
    ...
    for item in raw_overrides:
        ...
        value = value.strip()
        if key == "display_name":
            raise ValueError("display_name 固定等于稳定标准 Skill 名加版本号，不允许手动覆盖。")
        if key not in ALLOWED_INTERFACE_KEYS:
            allowed = ", ".join(sorted(ALLOWED_INTERFACE_KEYS))
            raise ValueError(f"未知 interface 字段：{key}。允许字段：{allowed}")
        overrides[key] = value
        if key != "short_description" and key not in optional_order:
...
def read_existing_display_version(skill_dir: Path, skill_name: str) -> str | None:
    yaml_path = skill_dir / "agents" / "openai.yaml"
    if not yaml_path.exists():
        return None
    text = yaml_path.read_text(encoding="utf-8")
    match = re.search(
        rf"^\s*display_name:\s*[\"']?{re.escape(skill_name)}_v(\d+\.\d+\.\d+)[\"']?\s*$",
        text,
        re.MULTILINE,
    )
...
def write_openai_yaml(skill_dir: Path, skill_name: str, raw_overrides: list[str], version: str | None = None) -> Path:
    if not SKILL_NAME_PATTERN.match(skill_name):
        raise ValueError("Skill name 必须是稳定名，例如 F01_原子拆解技能；版本号只写入 display_name。")
    display_version = version or read_existing_display_version(skill_dir, skill_name) or "1.0.0"
    if not VERSION_PATTERN.match(display_version):
        raise ValueError("display_name 版本必须是 MAJOR.MINOR.PATCH，例如 1.0.0。")
    overrides, optional_order = parse_interface_overrides(raw_overrides)
    display_name = f"{skill_name}_v{display_version}"
...
def main() -> int:
    ...
    parser.add_argument("--version", help="写入 display_name 的版本号；未传时保留现有版本或使用 1.0.0")
    parser.add_argument(
        "--interface",
        action="append",
        default=[],
        help="以 key=value 形式传入展示层覆盖项，可重复传入；display_name 固定等于标准 Skill 名",
    )
    ...
    try:
        output_path = write_openai_yaml(skill_dir, skill_name, args.interface, version=args.version)
