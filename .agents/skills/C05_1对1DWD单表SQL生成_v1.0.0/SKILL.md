...
1. 运行脚本生成 SQL 初稿（无任何前置思考），在 skill 根目录下执行：
   ...
   ```
   skill 根目录 = 本 SKILL.md 所在目录（`.../.agents/skills/C05_1对1DWD单表SQL生成_v1.0.0/`）
...
4. 处理时间戳字段（`⚠️ TIMESTAMP_FIELDS`）：
   - 按以下顺序查找 ODS CREATE TABLE SQL 文件：
     ...
     2. Doris 默认结果目录 `output/C03_Doris建表语句查询_v1.0.0/结果/`（文件名含当前 ODS 表名）
