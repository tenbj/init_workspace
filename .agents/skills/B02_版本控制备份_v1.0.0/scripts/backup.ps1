...
.DESCRIPTION
    ...
    - FOLDER  : .agents/skills/ folders or .system/ subfolders — snapshot with timestamp, source unchanged
    - CONFIG  : .agents/rules/ files and root entry files — copy with timestamp, source unchanged
...
if ($Mode -eq "CONFIG") {
    ...
    if ($relativePath -eq "AGENTS.md") {
        $backupDir = Join-Path $historyRoot "AGENTS"
    } elseif ($relativePath -eq "CLAUDE.md") {
        $backupDir = Join-Path $historyRoot "CLAUDE"
    } else {
