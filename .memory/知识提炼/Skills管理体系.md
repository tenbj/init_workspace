...
## 2026-05-11 · 课题研究 Skill 与根因分析

**核心观点**：
- B05 已从 `v1.0.0` 升级为 `v1.1.0`，把“根因分析”纳入显式研究流程。
- 原因类、诊断类、异常类、失败偏差类、机制追问类问题必须写“根因分析”章节。
- 根因分析采用固定链条：现象 → 直接原因 → 深层机制 → 根因判断 → 证据验证 → 修正建议。

**关键框架/模型**：
- 第一性原理：提供深层机制的基础来源。
- 根因判断：选择最上游、最可干预、解释力最强的主因；证据不足时必须写“当前最可能根因”或“暂不能确定”。
- 证据验证：列出能证实或证伪根因判断的观察、数据、日志、实验或文献。
- 修正建议：必须对准根因，不写泛泛建议。

**与其他话题的关联**：
- → B05 课题研究：通用方法论与健康养生专项方法论都已补充根因章节。
- → B01/B06：脚本 fallback 中的覆盖层 Skill 名称同步更新为 `B05_课题研究_v1.1.0`，避免 spec 缺失时回落到旧版本。
